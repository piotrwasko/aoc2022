...
class Task2 {
    fun solution(input: String): Int? {
        ...
        val cave = ArrayList<ArrayList<Point>>(maxY + 3)
        repeat(maxY + 2) {
            val l = ArrayList<Point>()
            repeat(IntRange(0, maxX * 2).count()) { l.add(Point.EMPTY) }
            cave.add(l)
        }
        val l = ArrayList<Point>()
        repeat(IntRange(0, maxX * 2).count()) { l.add(Point.ROCK) }
        cave.add(l)

        rocks.forEach {
            it.windowed(2).forEach { pair ->
                val left = min(pair[0][0], pair[1][0])
                val right = max(pair[0][0], pair[1][0])
        ...
        sandfall.add(500 to 0)

        while (sandfall.isNotEmpty()) {
            val point = sandfall.last()
            if (cave[point.second + 1][point.first].isEmpty()) {
